...
### 新增功能
- 新增ZIP文件导入导出功能，支持批量导入/导出歌词JSON文件和资源文件
- 新增"导出分享"功能，一键打包歌曲和相关资源文件
- 添加"保存全部"按钮，可同时保存专辑图和背景图路径
- 实现资源完整性检查，在导出时对缺失资源生成警告
- 优化安全机制，在关键操作前验证设备解锁状态

### 技术改进
- 增强路径安全验证，改进本地回环地址检测逻辑，支持IPv4映射地址
- 实现资源路径管理工具，自动收集JSON中引用的资源文件路径
- 完善文件扩展名验证逻辑，优化图片格式检查功能
- 为横屏大屏设备优化搜索工具栏响应式布局
- 提升歌词解析算法，改进路径处理逻辑，增加安全验证机制

### 功能优化
- 简化图片文件扩展名验证流程，统一使用hasValidImageExtension函数
- 优化搜索工具栏在不同设备上的显示效果
- 所有敏感操作（创建、删除、重命名、导出等）统一添加安全验证

## [v1.5.10] - 2025-11-07

### 新增功能
- 增强AMLL Web播放器集成功能，增加设置参数缓存能力
- 新增AMLL设置存储保护机制，防止关键配置意外丢失
- 在LyricSphere界面中增强AMLL规则编写功能，支持从现有设置构建样式查询参数

### 技术改进
- 优化AMLL Web播放器参数传递逻辑，支持从缓存读取设置并在新窗口中应用
- 重构amll-player.js代码，解决多处变量命名冲突问题
- 在Lyrics-style模板中新增页面卸载存储恢复功能
- 完善歌词文件格式检测逻辑，提升对多种歌词格式的支持

### 功能优化
- 改进openFamyliamCloud函数实现，转为异步函数并优化错误处理机制
- 添加TTML歌词临时转换功能，在规则编写时自动转换非TTML格式的歌词文件

## [v1.5.9] - 2025-11-02

### 新增功能
- 添加AMLL Web播放器集成，提供现代化的Web界面用于歌词播放和控制
- 新增AMLL Web播放器模板与静态资源路由，支持自定义基础URL配置
- 增强歌词动画页面的封面与背景图处理逻辑，增加临时覆盖参数支持

### 技术改进
- 优化TTML转换中的空白字符处理，改进时间戳提取兼容性
- 调整移动端媒体查询条件，增加竖屏模式适配
- 完善歌词文件扩展名自动检测功能，增强编辑体验
- 改进括号背景行检测逻辑，支持全角括号匹配
