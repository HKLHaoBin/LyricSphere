...
flowchart TD
    RP["原始路径 / URL 字符串"]
    ER["extract_resource_relative"]
    NP["_normalize_relative_path"]
    CS["检查 . 或 .. 段"]
    RP2["resolve_resource_path"]
    BC["检查 base 目录 boundary"]
    RT["返回路径"]
    ...
    %% 分层注释（文本说明，不使用 subgraph）
    click ER "Layer 1 Extraction: URL scheme & prefix"
    click NP "Layer 2 Normalization: clean segments"
    click CS "Layer 2 Normalization"
    click RP2 "Layer 3 Resolution: absolute path"
    click BC "Layer 3 Boundary check"
...
flowchart LR
    ...
    WSServer[WebSocket 服务器<br>WebSocket :11444 & AMLL 集成]
    ...
    %% FastAPI backend
    subgraph BACKEND [FastAPI 后端 backend.py]
        Router
        SongAPI
        LyricAPI
        ConvertAPI
...
sequenceDiagram
    ...
    participant RequestCtx as _request_context_middleware
    participant AuthCheck
    participant CheckAllow as is_request_allowed()
    participant ReqUnlock as require_unlocked_device()
    participant RouteHandler
    participant SessionProxy
    participant Session

    Client->>Middleware: HTTP 请求
    Middleware->>RequestCtx: 创建 RequestContext
    Middleware->>Middleware: 解析 body 和 form
    Middleware->>RouteHandler: before_request 钩子
    RouteHandler->>AuthCheck: 路由检查

    %% 设备已解锁 & 认证检查
    AuthCheck->>CheckAllow: 检查认证
    CheckAllow-->>AuthCheck: 允许/拒绝

    alt 认证允许
        AuthCheck->>SessionProxy: IPv4 映射 & 回环
        SessionProxy-->>AuthCheck: 本地绕过许可
        AuthCheck->>Session: 检查 device_unlocked
        Session-->>AuthCheck: 已解锁
        AuthCheck-->>Client: 返回允许
    else 认证失败
        AuthCheck-->>RouteHandler: 拒绝访问
        RouteHandler-->>Middleware: 403 Forbidden
        Middleware-->>Client: 错误响应 {"unlock_required": true}
    end
...
flowchart TD
    ...
    Normalize["normalize_relative_path"]
    Resolve["resolve_resource_path"]
    Validate["relative_to boundary check"]
    Result["Safe path (whitelisted)"]
    Error["ValueError (traversal)"]
...
flowchart TD
    ...
    STATIC["static 文件夹 静态资源"]
    SONGS["songs 文件夹 歌曲文件"]
    BACKUPS["backups 文件夹 版本历史"]
    LOGS["logs 文件夹 应用日志"]
    EXPORTS["exports 文件夹 导出包"]
    TEMPLATES["templates 文件夹 Jinja2 模板"]

    SongJSON["*.json 歌曲元数据"]
    Audio["*.mp3 / *.flac 音频文件"]
    Images["*.jpg / *.png 封面图像"]
    Fonts["*.ttf / *.otf 自定义字体"]
    BackupFiles["备份 文件名.timestamp"]
...
A: 使用 `封装工具.bat` 和 `封装配置*.json` 文件将应用程序打包为可执行文件。CI 发布流程见 [`.github/workflows/build-exe-release.yml`](.github/workflows/build-exe-release.yml)（`pyinstaller --noconfirm backend.spec` 与 smoke 测试）；贡献者细节见仓库根目录 [`AGENTS.md`](AGENTS.md) 的 **CI / Release** 小节。
