...
flowchart TD
    Input[用户输入路径字符串]
    Extract[extract_resource_relative() 解析 URL 并移除前缀]
    Normalize[_normalize_relative_path() 清理 .. 和 斜杠 验证段]
    Resolve[resolve_resource_path() 基础路径拼接 得到绝对路径]
    Validate[relative_to() 路径边界验证 是否允许]
    Result[安全 Path 对象 在白名单内]
    Error[ValueError 路径遍历 检测到 错误]
