...
```mermaid
flowchart TD
    Start["python backend.py"] --> ParseArgs["解析命令行参数\nport = sys.argv 第1个参数 或 5000"]
    ParseArgs --> CheckEnv["检查环境变量"]
    CheckEnv --> InitDirs["初始化目录结构\ncreate_directories"]
    ...
    CreateSongs --> InitBackend["初始化 FastAPI 应用\nbackend.py"]
    CreateBackups --> InitBackend
    CreateLogs --> InitBackend
    CreateExports --> InitBackend
    InitBackend --> SetupRoutes["注册 HTTP 路由\n60+ 端点"]
    SetupRoutes --> SetupWS["启动 WebSocket 服务器\n端口 11444"]
    SetupWS --> SetupSSE["初始化 SSE 流\n/amll/stream"]
    SetupSSE --> ChooseServer{"USE_WAITRESS\n== 1?"}
    ChooseServer -->|是| StartWaitress["启动 Waitress 服务器\n生产模式"]
    ChooseServer -->|否| StartUvicorn["启动 Uvicorn 服务器\n开发模式"]
    StartWaitress --> Ready["服务器就绪\nhttp://0.0.0.0:5000"]
    StartUvicorn --> Ready
...
```mermaid
flowchart TD
    Dashboard["LyricSphere.html\n主仪表板"]
    AMMLPlayer["Lyrics-style.HTML-AMLL-v1.HTML\nAMLL 播放器"]
    QuickEditor["lyrics_quick_editor.html\n快速编辑器"]
    OtherUI[其他 UI 模板]
    FastAPIApp["FlaskCompat (FastAPI)\nRESTful API 服务器"]
    Routes["60+ API 端点\n/songs, /translate, /convert"]
    Middleware["中间件栈\nCORS, 请求上下文, 会话"]
    LyricParsers["歌词解析器\nLRC/LYS/TTML/LQE"]
    ...
    SSEEndpoint["SSE 端点\n/amll/stream"]
    AnimationSync["动画配置同步\n/player/animation-config"]
    SongsDir["static/songs/\nJSON + 媒体文件"]
    BackupsDir["static/backups/\n7版本轮换"]
    ExportsDir["exports/\nZIP 包"]
    LogsDir["logs/\nupload.log"]
    DeviceAuth["设备认证\n可信设备管理"]
    PasswordProtection["密码保护\nbcrypt 哈希"]
    PathValidation["路径验证\nsanitize_filename, resolve_resource_path"]
...
flowchart TD
    ...
    FlaskCompatClass["FlaskCompat(FastAPI)\napp 实例"]
    RouteDecorator["@app.route()\nFlask 风格装饰器"]
    ContextVars["ContextVar 存储\n_request_context"]
    RequestContext["RequestContext\nrequest body + form + json"]
    RequestProxy["RequestProxy (request)\n全局访问点"]
    SessionProxy["SessionProxy (session)\n会话管理"]
    ContextMiddleware["_request_context_middleware\n上下文注入"]
    BeforeRequest["@app.before_request\nCORS 预检处理器"]
    AfterRequest["@app.after_request\nCORS 头应用器"]
...
flowchart TD
    ...
    SaveUploadFile["save_upload_file\n异步保存"]
    SaveWithMeta["save_upload_file_with_meta\n大小 + MD5 计算"]
    SanitizeFilename["sanitize_filename\n移除危险字符"]
    ResolveResourcePath["resolve_resource_path\n解析 + 验证"]
    BuildBackupPath["build_backup_path\n时间戳 + 哈希"]
    RotateBackups["备份轮换\n最多 7 版本"]
    SONGS_DIR["SONGS_DIR\nstatic/songs/"]
...
```mermaid
flowchart TD
    MainDashboard["LyricSphere.html\n歌曲管理仪表板"]
    AMMLPlayerV1["Lyrics-style.HTML-AMLL-v1.HTML\n高级 AMLL 播放器"]
    QuickEditorUI["lyrics_quick_editor.html\n快速歌词编辑器"]
    BasicPlayer["Lyrics-style.HTML\n基础播放器"]
    AnimatePlayer["Lyrics-style.HTML-Animate.HTML\n动画展示"]
    AMLLIntegration["Lyrics-style.HTML-AMLL-v1-test.HTML\nAMLL 测试播放器"]
    APIServer["backend.py\nFlaskCompat 应用"]
...
```mermaid
flowchart TD
    FrontendParams["前端动画参数\nentry/move/exit 持续时间"]
    AnimConfigRoute["/player/animation-config\nPOST 处理器"]
    DefaultSync["默认：600ms\n验证 + 规范化"]
    UseComputedFlag["useComputedDisappear\n切换标志"]
    ComputeDisappear["compute_disappear_times\n计算行退出时序"]
    TimestampAlign["时间戳对齐\n与下一行对齐"]
    BrowserRender["浏览器渲染器\n应用动画"]
    AMLLRender["AMLL 渲染器\n处理规则"]
...
flowchart TD
    ...
    ResourceMap["RESOURCE_DIRECTORIES\n映射字典"]
...
flowchart TD
    ...
    CreateBackup[创建备份决策]
    OriginalName["原始文件名\nexample_song.json"]
    NormalizeLength["_normalize_backup_basename\n最多 255 字符"]
    AddTimestamp["添加时间戳\n.20240115_143052"]
    HashLongNames["如果太长则哈希\nSHA1 后 8 位]
    BackupDirStore["BACKUP_DIR\nstatic/backups/"]
    CountVersions["计数现有版本\n按前缀匹配"]
    DeleteOldest["删除最旧的\n如果计数 > 7"]
...
```mermaid
flowchart TD
    CORSMiddleware["CORS 处理器\n_match_cors_origin"]
    PreflightHandler["预检处理器\nhandle_cors_preflight"]
    HeaderApplier["头应用器\napply_cors_headers"]
    DeviceCheck[设备状态检查]
    TrustedList["可信设备\n基于 localStorage"]
    PasswordPrompt["密码验证\nbcrypt.checkpw"]
    SanitizeFn["sanitize_filename\nSAFE_FILENAME_PATTERN"]
    ResolvePathFn["resolve_resource_path\n边界检查"]
    NormalizePathFn["_normalize_relative_path\n段验证"]
    WhitelistCheck["RESOURCE_DIRECTORIES\n白名单验证"]
    SessionMiddleware["SessionMiddleware\nStarlette"]
    SessionProxy["SessionProxy (session)\n上下文感知访问"]
    SecretKey["app.secret_key\n会话加密"]
    IPv4MappedCheck["IPv4 映射检测\n::ffff:127.0.0.1"]
    LoopbackDetect["回环检测\n本地地址检查"]
    OperationGating["操作门控\n读取/写入/关键"]
    UploadLog["upload.log\n操作跟踪"]
    ErrorLog["错误日志\n失败操作"]
    TimedRotation["TimedRotatingFileHandler\n日志轮换"]
...
```mermaid
flowchart TD
    BuildClient["build_openai_client\n弹性 SSL 设置"]
    CleanupSSL["cleanup_missing_ssl_cert_env\n移除无效环境变量"]
    CertifiFallback["certifi.where()\nCA 包回退"]
    DisableSSLLastResort["httpx.Client(verify=False)\n最后回退"]
    DeepSeek["DeepSeek API\ndeepseek-reasoner"]
    OpenAI["OpenAI API\nGPT 模型"]
    OpenRouter["OpenRouter API\n多模型网关"]
    Together["Together API\n开放模型"]
    Groq["Groq API\n快速推理"]
    TranslateEndpoint["/translate_lyrics\n路由处理器"]
    PromptConstruction["提示构建\n系统 + 用户消息"]
    ThinkingModel["可选思维阶段\n预分析模型"]
...
```mermaid
flowchart TD
    FontMetaTag["font-family 标签\n在歌词内容中"]
    ScriptDetection["脚本检测\nCJK vs 拉丁"]
    FontMapping["字体系列映射\n按脚本选择"]
    LocalFonts["本地文件\n/songs 或 /fonts"]
    GoogleFonts["Google Fonts API\n动态加载"]
    CDNFonts["字体 CDN\n回退源"]
    SystemFonts["系统字体\n最终回退"]
    AvailabilityCheck["字体可用性检查\ndocument.fonts.check"]
    SpecialFontMode["特殊字体模式\n纯色渲染"]
    SyllableAnimation["逐音节动画\n优化渲染"]
...
```mermaid
flowchart TD
    DevScript["python backend.py\n直接执行"]
    DevPath["BASE_PATH = file.parent\n脚本目录"]
    DevServer["FastAPI 开发服务器\n或 Waitress (USE_WAITRESS=1)"]
    FrozenExe["backend.exe\nPyInstaller 包"]
    FrozenPath["BASE_PATH = sys.executable.parent\n可执行文件目录"]
    WaitressServer["Waitress WSGI 服务器\n生产级"]
    EnvVars["环境变量\nPORT, PUBLIC_BASE_URL 等"]
    ConfigDict["app.config 字典\n运行时设置"]
    LocalNetwork["本地网络\nhttp://IP:PORT"]
    PublicURL["公共 URL\n配置的基础 URL"]
...
flowchart TD
    ...
    SongAPI["歌曲管理 API\n/api/songs/*"]
    LyricAPI["歌词 API\n/lyrics, /api/songs/*/lyrics"]
    ConvertAPI["格式转换 API\n/convert_*, /merge_*, /export_*"]
    TranslateAPI["AI 翻译 API\n/translate_lyrics, /api/ai/*"]
    ImportExport["导入/导出 API\n/api/import, /api/export/*"]
    BackupAPI["备份 API\n/api/songs/*/backups/"]
    AuthAPI["认证 API\n/auth/*"]
    ConfigAPI["配置 API\n/api/settings, /player/animation-config"]
    EditorAPI["快速编辑器 API\n/quick-editor/api/*"]
    RealtimeAPI["实时 API\n/amll/stream, /amll/state"]
    WSServer["WebSocket :11444\nAMLL 集成"]
...
flowchart TD
    Input["用户输入\n路径字符串"]
    Extract["extract_resource_relative()\n解析 URL，移除前缀"]
    Normalize["_normalize_relative_path()\n清理 .., /，验证段"]
    Resolve["resolve_resource_path()\n连接基础，解析绝对"]
    Validate["路径验证\nrelative_to() 检查"]
    Result["安全路径对象\n在白名单内"]
    Error["ValueError\n检测到路径遍历"]
...
flowchart TD
    ...
    Images["*.jpg, *.png\n(封面图像)"]
    Fonts["*.ttf, *.otf\n(自定义字体)"]
    BackupFiles["filename.timestamp\n(版本化备份)"]
    UploadLog["upload.log\n(活动日志)"]
    ZipExports["*.zip\n(导出包)"]
...
```mermaid
flowchart TD
    UserInput["用户输入 (URL / 路径 / 文件名)"]
    ExtractRelative["extract_resource_relative() 解析相对路径"]
    CheckScheme["检查 URL scheme/netloc"]
    RemovePrefix["移除资源前缀 (如 songs/)"]
    NormalizePath["_normalize_relative_path() 清理路径段"]
    CheckDots["拒绝 .. 和 . 段"]
    JoinSegments["连接有效段"]
    ResolveAbs["resolve_resource_path() 转为绝对路径"]
    ResolveSymlinks["解析符号链接"]
    BoundaryCheck["验证在基础目录内"]
    SanitizeFilename["sanitize_filename() 移除非法字符"]
    CleanPattern["应用 SAFE_FILENAME_PATTERN"]
    ReplaceQuotes["替换引号为全角"]
    SafePath["安全绝对路径 (用于 I/O)"]
    ...
    subgraph Layer4Sanitization["第4层：清理"]
        SanitizeFilename --> CleanPattern --> ReplaceQuotes
    end

    subgraph Layer3Resolution["第3层：解析"]
        ResolveAbs --> ResolveSymlinks --> BoundaryCheck
    end

    subgraph Layer2Normalization["第2层：规范化"]
        NormalizePath --> CheckDots --> JoinSegments
    end
...
Sanitizer["sanitize_ttml_content()\n白名单过滤"]
Extractor["元数据提取器\n时间戳、属性"]
Output["解析结构\n行 + 元数据"]
Input --> Parser
Parser --> Sanitizer
...
```mermaid
flowchart TD
SyllableTiming["音节时间数据\n(start, duration)"]
AnimParams["动画参数\n(entry, move, exit durations)"]
CalcAppear["计算出现时间\n(start - entry_duration)"]
CalcDisappear["计算消失时间\n(start + duration + exit_duration)"]
NextLineCheck["检查下一行时间"]
GapCalc["计算行之间的间隙"]
AdjustOverlap["调整重叠\n(防止视觉冲突)"]
AlignedTimestamps["对齐的时间戳\n(appear, disappear)"]
...
ThinkingStage["思考模型调用\n歌词分析"]
AnalysisResult["分析结果\n主题、情绪、上下文"]
EnhancedPrompt["增强翻译提示\n包含分析上下文"]
MainTranslation["主翻译模型\n上下文感知翻译"]
TransOutput["翻译输出"]
