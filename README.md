...
```mermaid
flowchart TD
    MainDashboard["LyricSphere.html 歌曲管理仪表板"]
    AMMLPlayerV1["Lyrics-style.HTML-AMLL-v1.HTML 高级 AMLL 播放器"]
    QuickEditorUI["lyrics_quick_editor.html 快速歌词编辑器"]
    BasicPlayer["Lyrics-style.HTML 基础播放器"]
    AnimatePlayer["Lyrics-style.HTML-Animate.HTML 动画展示"]
    AMLLIntegration["Lyrics-style.HTML-AMLL-v1-test.HTML AMLL 测试播放器"]
    APIServer["backend.py FlaskCompat 应用"]
    MonacoEditor["Monaco Editor 代码编辑"]
    AMLLAPI["AMLL 集成 WebSocket + SSE"]
...
```mermaid
flowchart TD
    FrontendParams["前端动画参数 entry/move/exit 持续时间"]
    AnimConfigRoute["/player/animation-config POST 处理器"]
    DefaultSync["默认：600ms 验证 + 规范化"]
    UseComputedFlag["useComputedDisappear 切换标志"]
    ComputeDisappear["compute_disappear_times 计算行退出时序"]
    TimestampAlign["时间戳对齐 与下一行对齐"]
    BrowserRender["浏览器渲染器 应用动画"]
    AMLLRender["AMLL 渲染器 处理规则"]
...
```mermaid
flowchart TD
    FontMetaTag["font-family 标签 在歌词内容中"]
    ScriptDetection["脚本检测 CJK vs 拉丁"]
    FontMapping["字体系列映射 按脚本选择"]
    LocalFonts["本地文件 /songs 或 /fonts"]
    GoogleFonts["Google Fonts API 动态加载"]
    CDNFonts["字体 CDN 回退源"]
    SystemFonts["系统字体 最终回退"]
    AvailabilityCheck["字体可用性检查 document.fonts.check"]
    SpecialFontMode["特殊字体模式 纯色渲染"]
    SyllableAnimation["逐音节动画 优化渲染"]
...
```mermaid
flowchart TD
    IncomingRequest["传入 HTTP 请求 StarletteRequest"]
...
flowchart TD
    ...
    SongAPI["歌曲管理 API /api/songs/*"]
    LyricAPI["歌词 API /lyrics, /api/songs/*/lyrics"]
    ConvertAPI["格式转换 API /convert_*, /merge_*, /export_*"]
    TranslateAPI["AI 翻译 API /translate_lyrics, /api/ai/*"]
    ImportExport["导入/导出 API /api/import, /api/export/*"]
    BackupAPI["备份 API /api/songs/*/backups/"]
    AuthAPI["认证 API /auth/*"]
    ConfigAPI["配置 API /api/settings, /player/animation-config"]
    EditorAPI["快速编辑器 API /quick-editor/api/*"]
    RealtimeAPI["实时 API /amll/stream, /amll/state"]
    WSServer["WebSocket :11444 AMLL 集成"]
...
flowchart TD
    Input["用户输入 路径字符串"]
    Extract["extract_resource_relative() 解析 URL，移除前缀"]
    Normalize["_normalize_relative_path() 清理 .., /，验证段"]
    Resolve["resolve_resource_path() 连接基础，解析绝对"]
    Validate["路径验证 relative_to() 检查"]
    Result["安全路径对象 在白名单内"]
    Error["ValueError 检测到路径遍历"]
...
flowchart TD
    ...
    Images["*.jpg, *.png (封面图像)"]
    Fonts["*.ttf, *.otf (自定义字体)"]
    BackupFiles["filename.timestamp (版本化备份)"]
    UploadLog["upload.log (活动日志)"]
    ZipExports["*.zip (导出包)"]
...
```mermaid
flowchart TD
    OriginalFile["原始文件 song_lyrics.json"]
    CheckLength["文件名长度 > 255 - 后缀?"]
    UseDirect["直接命名 song_lyrics.json.20240115_143022"]
    HashAndTruncate["哈希 + 截断"]
    ComputeHash["SHA1 哈希 → 8 字符"]
    TruncateStem["截断干以适应"]
    BuildName["stem_hash.json.timestamp"]
    BackupPath["备份路径 static/backups/filename.timestamp"]
    RotateOld["轮换旧备份 保留最近 7 个"]
...
LYSMarkers["LYS 模式 text(start,dur) 6-7-8 标记"]
...
CalcAppear["计算出现时间 (start - entry_duration)"]
CalcDisappear["计算消失时间 (start + duration + exit_duration)"]
