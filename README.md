...
A: 使用 `封装工具.bat` 和 `封装配置*.json` 文件将应用程序打包为可执行文件。CI 仅负责构建与发布 zip，**请在本地解压 release 布局后做落盘验证**。流程见 [`.github/workflows/build-exe-release.yml`](.github/workflows/build-exe-release.yml)；细节见 [`AGENTS.md`](AGENTS.md) 的 **CI / Release** 小节。
