...
```mermaid
flowchart TD
    Start[python backend.py] --> ParseArgs[解析命令行参数<br/>port = sys.argv[1] or 5000]
    ParseArgs --> CheckEnv[检查环境变量]
    CheckEnv --> InitDirs[初始化目录结构<br/>create_directories]
    ...
    CreateSongs --> InitBackend[初始化 FastAPI 应用<br/>backend.py]
    CreateBackups --> InitBackend
    CreateLogs --> InitBackend
    CreateExports --> InitBackend
    InitBackend --> SetupRoutes[注册 HTTP 路由<br/>60+ 端点]
    SetupRoutes --> SetupWS[启动 WebSocket 服务器<br/>端口 11444]
    SetupWS --> SetupSSE[初始化 SSE 流<br/>/amll/stream]
    SetupSSE --> ChooseServer{USE_WAITRESS<br/>== 1?}
    ChooseServer -->|是| StartWaitress[启动 Waitress 服务器<br/>生产模式]
    ChooseServer -->|否| StartUvicorn[启动 Uvicorn 服务器<br/>开发模式]
    StartWaitress --> Ready[服务器就绪<br/>http://0.0.0.0:5000]
    StartUvicorn --> Ready
...
```mermaid
flowchart TD
    Dashboard[LyricSphere.html<br/>主仪表板]
    AMMLPlayer[Lyrics-style.HTML-AMLL-v1.HTML<br/>AMLL 播放器]
    QuickEditor[lyrics_quick_editor.html<br/>快速编辑器]
    OtherUI[其他 UI 模板]
    FastAPIApp[FlaskCompat (FastAPI)<br/>RESTful API 服务器]
    Routes[60+ API 端点<br/>/songs, /translate, /convert]
    Middleware[中间件栈<br/>CORS, 请求上下文, 会话]
    LyricParsers[歌词解析器<br/>LRC/LYS/TTML/LQE]
    FormatConverters[格式转换器<br/>双向转换]
    AIEngine[AI 翻译引擎<br/>多提供商支持]
    FileManager[文件管理系统<br/>上传, 备份, 导出]
    WebSocketServer[WebSocket 服务器<br/>端口 11444]
    SSEEndpoint[SSE 端点<br/>/amll/stream]
    AnimationSync[动画配置同步<br/>/player/animation-config]
    SongsDir[static/songs/<br/>JSON + 媒体文件]
    BackupsDir[static/backups/<br/>7版本轮换]
    ExportsDir[exports/<br/>ZIP 包]
    LogsDir[logs/<br/>upload.log]
    DeviceAuth[设备认证<br/>可信设备管理]
    PasswordProtection[密码保护<br/>bcrypt 哈希]
    PathValidation[路径验证<br/>sanitize_filename, resolve_resource_path]
    CORSHandler[CORS 处理器<br/>跨域支持]
...
```mermaid
flowchart TD
    StarletteRequest[StarletteRequest<br/>传入 HTTP]
    FlaskCompatClass[FlaskCompat(FastAPI)<br/>app 实例]
    RouteDecorator[@app.route()<br/>Flask 风格装饰器]
    ContextVars[ContextVar 存储<br/>_request_context]
    RequestContext[RequestContext<br/>request body + form + json]
    RequestProxy[RequestProxy (request)<br/>全局访问点]
    SessionProxy[SessionProxy (session)<br/>会话管理]
    ContextMiddleware[_request_context_middleware<br/>上下文注入]
    BeforeRequest[@app.before_request<br/>CORS 预检处理器]
    AfterRequest[@app.after_request<br/>CORS 头应用器]
...
```mermaid
flowchart TD
    UploadFile[UploadFile<br/>FastAPI 原生]
    FileStorageAdapter[FileStorageAdapter<br/>包装类]
    FilesWrapper[FilesWrapper<br/>多文件容器]
    SaveUploadFile[save_upload_file<br/>异步保存]
    SaveWithMeta[save_upload_file_with_meta<br/>大小 + MD5 计算]
    SanitizeFilename[sanitize_filename<br/>移除危险字符]
    ResolveResourcePath[resolve_resource_path<br/>解析 + 验证]
    BuildBackupPath[build_backup_path<br/>时间戳 + 哈希]
    RotateBackups[备份轮换<br/>最多 7 版本]
...
```mermaid
flowchart TD
    MainDashboard[LyricSphere.html<br/>歌曲管理仪表板]
    AMMLPlayerV1[Lyrics-style.HTML-AMLL-v1.HTML<br/>高级 AMLL 播放器]
    QuickEditorUI[lyrics_quick_editor.html<br/>快速歌词编辑器]
    BasicPlayer[Lyrics-style.HTML<br/>基础播放器]
    AnimatePlayer[Lyrics-style.HTML-Animate.HTML<br/>动画展示]
    AMLLIntegration[Lyrics-style.HTML-AMLL-v1-test.HTML<br/>AMLL 测试播放器]
    APIServer[backend.py<br/>FlaskCompat 应用]
    MonacoEditor[Monaco Editor<br/>代码编辑]
    AMLLAPI[AMLL 集成<br/>WebSocket + SSE]
    GoogleFonts[Google Fonts<br/>动态加载]
    ...
    subgraph PlayerVariants[播放器变体]
        BasicPlayer
        AnimatePlayer
        AMLLIntegration
    end

    subgraph PrimaryInterfaces[主要界面]
        MainDashboard
        AMMLPlayerV1
        QuickEditorUI
    end
...
```mermaid
flowchart TD
    FrontendParams[前端动画参数<br/>entry/move/exit 持续时间]
    AnimConfigRoute[/player/animation-config<br/>POST 处理器]
    DefaultSync[默认：600ms<br/>验证 + 规范化]
    UseComputedFlag[useComputedDisappear<br/>切换标志]
    ComputeDisappear[compute_disappear_times<br/>计算行退出时序]
    TimestampAlign[时间戳对齐<br/>与下一行对齐]
    BrowserRender[浏览器渲染器<br/>应用动画]
    AMLLRender[AMLL 渲染器<br/>处理规则]
...
```mermaid
flowchart TD
    BasePath[BASE_PATH<br/>应用根目录]
    StaticDir[static/<br/>Web 可访问文件]
    SongsDir[static/songs/<br/>JSON + 音频 + 图像]
    BackupsDir[static/backups/<br/>7版本轮换]
...
flowchart TD
    FileWrite[文件写入操作]
    CreateBackup[创建备份决策]
    OriginalName[原始文件名<br/>example_song.json]
    NormalizeLength[_normalize_backup_basename<br/>最多 255 字符]
    AddTimestamp[添加时间戳<br/>.20240115_143052]
    HashLongNames[如果太长则哈希<br/>SHA1[:8] 后缀]
    BackupDirStore[BACKUP_DIR<br/>static/backups/]
    CountVersions[计数现有版本<br/>按前缀匹配]
    DeleteOldest[删除最旧的<br/>如果计数 > 7]
...
```mermaid
flowchart TD
    CORSMiddleware[CORS 处理器<br/>_match_cors_origin]
    PreflightHandler[预检处理器<br/>handle_cors_preflight]
    HeaderApplier[头应用器<br/>apply_cors_headers]
    DeviceCheck[设备状态检查]
    TrustedList[可信设备<br/>基于 localStorage]
    PasswordPrompt[密码验证<br/>bcrypt.checkpw]
    SanitizeFn[sanitize_filename<br/>SAFE_FILENAME_PATTERN]
    ResolvePathFn[resolve_resource_path<br/>边界检查]
    NormalizePathFn[_normalize_relative_path<br/>段验证]
    WhitelistCheck[RESOURCE_DIRECTORIES<br/>白名单验证]
    SessionMiddleware[SessionMiddleware<br/>Starlette]
    SessionProxy[SessionProxy (session)<br/>上下文感知访问]
    SecretKey[app.secret_key<br/>会话加密]
    IPv4MappedCheck[IPv4 映射检测<br/>::ffff:127.0.0.1]
    LoopbackDetect[回环检测<br/>本地地址检查]
    OperationGating[操作门控<br/>读取/写入/关键]
    UploadLog[upload.log<br/>操作跟踪]
    ErrorLog[错误日志<br/>失败操作]
    TimedRotation[TimedRotatingFileHandler<br/>日志轮换]
...
```mermaid
flowchart TD
    BuildClient[build_openai_client<br/>弹性 SSL 设置]
    CleanupSSL[cleanup_missing_ssl_cert_env<br/>移除无效环境变量]
    CertifiFallback[certifi.where()<br/>CA 包回退]
    DisableSSLLastResort[httpx.Client(verify=False)<br/>最后回退]
    DeepSeek[DeepSeek API<br/>deepseek-reasoner]
    OpenAI[OpenAI API<br/>GPT 模型]
    OpenRouter[OpenRouter API<br/>多模型网关]
    Together[Together API<br/>开放模型]
    Groq[Groq API<br/>快速推理]
    TranslateEndpoint[/translate_lyrics<br/>路由处理器]
    PromptConstruction[提示构建<br/>系统 + 用户消息]
    ThinkingModel[可选思维阶段<br/>预分析模型]
    StreamingResponse[流式响应<br/>实时块]
    TimestampSync[时间戳同步]
    LineValidation[行数验证]
    IssueDetection[问题检测<br/>未翻译/错误行]
...
```mermaid
flowchart TD
    FontMetaTag[[font-family:...] 标签<br/>在歌词内容中]
    ScriptDetection[脚本检测<br/>CJK vs 拉丁]
    FontMapping[字体系列映射<br/>按脚本选择]
    LocalFonts[本地文件<br/>/songs 或 /fonts]
    GoogleFonts[Google Fonts API<br/>动态加载]
    CDNFonts[字体 CDN<br/>回退源]
    SystemFonts[系统字体<br/>最终回退]
    AvailabilityCheck[字体可用性检查<br/>document.fonts.check]
    SpecialFontMode[特殊字体模式<br/>纯色渲染]
    SyllableAnimation[逐音节动画<br/>优化渲染]
...
```mermaid
flowchart TD
    DevScript[python backend.py<br/>直接执行]
    DevPath[BASE_PATH = file.parent<br/>脚本目录]
    DevServer[FastAPI 开发服务器<br/>或 Waitress (USE_WAITRESS=1)]
    ...
    subgraph ProductionMode[生产模式（冻结）]
        FrozenExe
        FrozenPath
        WaitressServer
    end

    subgraph DevelopmentMode[开发模式]
        DevScript
        DevPath
        DevServer
    end
...
```mermaid
flowchart TD
    IncomingRequest[传入 HTTP 请求<br/>StarletteRequest]
    ContextMiddleware[_request_context_middleware]
    BodyParse[解析 body 和 form]
    ...
    subgraph AfterRequestHooks[请求后钩子]
        AfterHooks
        CorsHeaders
    end

    subgraph RouteHandler[路由处理器]
        EndpointFunc
        RunSyncThread
        NormalizeResp
    end

    subgraph BeforeRequestHooks[请求前钩子]
        BeforeHooks
        CorsCheck
    end
...
flowchart TD
    ...
    LyricAPI["歌词 API<br/>/lyrics, /api/songs/*/lyrics"]
    ConvertAPI["格式转换 API<br/>/convert_*, /merge_*, /export_*"]
    TranslateAPI["AI 翻译 API<br/>/translate_lyrics, /api/ai/*"]
    ImportExport["导入/导出 API<br/>/api/import, /api/export/*"]
    BackupAPI["备份 API<br/>/api/songs/*/backups/"]
    AuthAPI["认证 API<br/>/auth/*"]
    ConfigAPI["配置 API<br/>/api/settings, /player/animation-config"]
    EditorAPI["快速编辑器 API<br/>/quick-editor/api/*"]
    RealtimeAPI["实时 API<br/>/amll/stream, /amll/state"]
    ...
    subgraph FastAPIBackend[FastAPI 后端 - backend.py]
        Router
        SongAPI
        LyricAPI
        ConvertAPI
...
flowchart TD
    ...
    Normalize["_normalize_relative_path()<br/>清理 .., /，验证段"]
    Resolve["resolve_resource_path()<br/>连接基础，解析绝对"]
    Validate["路径验证<br/>relative_to() 检查"]
    Result["安全路径对象<br/>在白名单内"]
...
```mermaid
flowchart TD
    UserInput["用户输入<br/>(URL, 路径, 文件名)"]
    ExtractRelative["extract_resource_relative()<br/>解析并提取相对路径"]
    CheckScheme["检查 URL scheme/netloc"]
    RemovePrefix["移除资源前缀<br/>(例如 'songs/')"]
    NormalizePath["_normalize_relative_path()<br/>清理路径段"]
    CheckDots["拒绝 '..' 和 '.'<br/>段"]
    JoinSegments["连接有效段"]
    ResolveAbs["resolve_resource_path()<br/>转换为绝对路径"]
    ResolveSymlinks["解析符号链接"]
    BoundaryCheck["验证在基础目录内"]
    SanitizeFilename["sanitize_filename()<br/>移除非法字符"]
    CleanPattern["应用 SAFE_FILENAME_PATTERN"]
    ReplaceQuotes["替换引号为全角"]
    SafePath["安全绝对路径<br/>(准备好进行 I/O)"]
    ...
    subgraph Layer4Sanitization["第4层：清理"]
        SanitizeFilename
        CleanPattern
        ReplaceQuotes
    ...
    subgraph Layer3Resolution["第3层：解析"]
        ResolveAbs
        ResolveSymlinks
        BoundaryCheck
    ...
    subgraph Layer2Normalization["第2层：规范化"]
        NormalizePath
        CheckDots
        JoinSegments
...
flowchart TD
    ...
    UseDirect["直接命名<br/>song_lyrics.json.20240115_143022"]
    HashAndTruncate["哈希 + 截断"]
    ComputeHash["SHA1 哈希 → 8 字符"]
    TruncateStem["截断干以适应"]
    BuildName["stem_hash.json.timestamp"]
    BackupPath["备份路径<br/>static/backups/filename.timestamp"]
...
flowchart TD
    Client["客户端浏览器"]
    DeviceID["设备指纹<br/>IP + User-Agent"]
    ...
    TrustedList["可信设备列表"]
    SessionStore["会话存储<br/>SessionMiddleware"]
    PasswordHash["密码哈希<br/>bcrypt"]
    CreateOp["创建歌曲"]
    DeleteOp["删除歌曲"]
    EditOp["快速编辑器"]
    ExportOp["导出/导入"]
...
```mermaid
flowchart TD
    Operation["受保护操作<br/>create/edit/delete/export"]
...
LRC["LRC 格式<br>(行级时间)"]
LYS["LYS 格式<br>(音节级时间)"]
TTML["TTML 格式<br>(Apple 风格 XML)"]
LQE["LQE 格式<br>(合并歌词+翻译)"]
LYS --> TTML
LRC --> TTML
LYS --> LQE
LRC --> LQE
...
```mermaid
flowchart TD
Input["输入内容<br>(文件或字符串)"]
ExtCheck["扩展名检查"]
ContentCheck["内容模式检查"]
TTMLMarkers["XML 标记:<br>ttm:role, ttm:agent"]
LYSMarkers["LYS 模式:<br>text(start,dur)<br>[6][7][8] 标记"]
LRCMarkers["LRC 模式:<br>[MM:SS.mmm]<br>[ar:] [ti:] 标签"]
LQEMarkers["LQE 结构:<br>合并格式<br>多轨道"]
TTMLParser["TTMLParser<br>minidom.parseString"]
LYSParser["LYSParser<br>音节提取"]
LRCParser["LRCParser<br>行提取"]
LQEParser["LQEParser<br>多轨道"]
Input --> ExtCheck
Input --> ContentCheck
...
MarkerDetect["检测特殊标记"]
TextExtract["提取歌词文本"]
StripBrackets["strip_brackets<br>已启用?"]
...
Syllable["音节对象"]
StartTime["startTime: number<br>(毫秒)"]
EndTime["endTime: number<br>(毫秒)"]
Text["text: string<br>(音节内容)"]
Root --> Lines
Lines --> Line
Line --> Words
Words --> Word
...
ExtractData["提取:<br>- text<br>- startTime<br>- endTime"]
CheckMarkers["检查特殊标记:<br>[2][5][6][7][8]"]
BuildResult["构建解析结构"]
Return["返回解析的行"]
Error["引发 ValueError"]
Input --> ParseJSON
...
p1["段落 <p><br>(歌词行)"]
p2["段落 <p><br>(歌词行)"]
span1["文本段 <span><br>(begin/end 属性)"]
span2["文本段 <span><br>(begin/end 属性)"]
span3["文本段 <span><br>(begin/end 属性)"]
tt --> body
body --> div
div --> p1
div --> p2
...
Sanitizer["sanitize_ttml_content()<br>白名单过滤"]
Extractor["元数据提取器<br>时间戳、属性"]
Output["解析结构<br>行 + 元数据"]
Input --> Parser
Parser --> Sanitizer
Sanitizer --> Extractor
Extractor --> Output
...
Input["输入内容"]
ParseJSON["json.loads()"]
CheckStructure["有 'original'<br>和 'translation'?"]
CheckString["两个字段<br>都是字符串?"]
ReturnLQE["返回 'lqe'"]
TryOther["尝试其他格式"]
Input --> ParseJSON
...
ValidateFields["验证 'original'<br>和 'translation' 字段"]
...
sequenceDiagram
  ...
  Backend API->>AI Translation Service: 流式传输翻译请求
  AI Translation Service-->>Backend API: 翻译歌词
  Backend API->>Backend API: lys_to_lqe(original, translation)
  Backend API->>File System: 保存 LQE 文件
  Backend API-->>Edit Modal: 返回 LQE 内容
  Edit Modal->>Edit Modal: 更新编辑器显示
```
...
```mermaid
flowchart TD
SyllableTiming["音节时间数据<br>(start, duration)"]
AnimParams["动画参数<br>(entry, move, exit durations)"]
CalcAppear["计算出现时间<br>(start - entry_duration)"]
CalcDisappear["计算消失时间<br>(start + duration + exit_duration)"]
NextLineCheck["检查下一行时间"]
GapCalc["计算行之间的间隙"]
AdjustOverlap["调整重叠<br>(防止视觉冲突)"]
AlignedTimestamps["对齐的时间戳<br>(appear, disappear)"]
...
ThinkingStage["思考模型调用<br>歌词分析"]
AnalysisResult["分析结果<br>主题、情绪、上下文"]
EnhancedPrompt["增强翻译提示<br>包含分析上下文"]
MainTranslation["主翻译模型<br>上下文感知翻译"]
TransOutput["翻译输出"]
DirectTranslation["直接翻译<br>标准提示"]
